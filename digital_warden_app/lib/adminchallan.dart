...
class FeetchDataPagee extends StatefulWidget {
  @override
  _FeetchDataPageeState createState() => _FeetchDataPageeState();
}

class _FeetchDataPageeState extends State<FeetchDataPagee> {
  ...
  void _clearAll() {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text("Confirm Clear", style: TextStyle(color: Colors.red)),
          content: Text("Are you sure you want to clear all data?", style: TextStyle(fontSize: 16)),
          actions: [
            TextButton(
            ...
            TextButton(
              ...
              onPressed: () async {
                try {
                  await database.child('Records').remove();
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('All data has been cleared.', style: TextStyle(color: Colors.green))),
                  );
                } catch (e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Failed to clear data. Please try again.', style: TextStyle(color: Colors.red))),
                  );
                }
                Navigator.of(context).pop();
              },
